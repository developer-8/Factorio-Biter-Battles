...
local function draw_suspend_gui(player)
	...
	local l = t.add({
		type = "label",
		caption = "Suspend " .. global.suspend_target .. " ?\t" .. global.suspend_time_left .. "s",
	})
		...
		if total_votes > 0 then
			...
			if result >= 75 and total_votes > 1 then
				...
				Server.to_banned_embed(table.concat({
					global.suspend_target
						.. " was suspended ( "
						.. result
						.. " %)"
						.. ", vote started by "
						.. global.suspend_starter,
				}))
...
decrement_timer_token = Token.register(function()
	...
	for _, player in pairs(game.connected_players) do
		if player.gui.top.suspend_frame and global.suspend_target ~= nil then
			player.gui.top.suspend_frame.suspend_table.children[1].caption = "Suspend "
				.. global.suspend_target
				.. " ?\t"
				.. suspend_time_left
				.. "s"
...
local function suspend_player(cmd)
	...
	if cmd.parameter then
		...
		if victim and victim.valid then
			...
			game.print(
				killer.name .. " has started a vote to suspend " .. global.suspend_target .. " , vote in top of screen"
			)
