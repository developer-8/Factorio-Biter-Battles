...
local function fill_starter_chests(surface)
    ...
    if storage.packchest2N then
        storage.packchest2N.destroy()
    end
    if storage.packchest2S then
        storage.packchest2S.destroy()
    end
    storage.packchest2N =
        surface.create_entity({ name = 'steel-chest', position = { x = _posX, y = -_posY }, force = 'north' })
    storage.packchest2S =
        surface.create_entity({ name = 'steel-chest', position = { x = _posX, y = _posY - 1 }, force = 'south' })
...
local function tournament1vs1_mode_toggle(cmd)
    ...
    game.print(
        '1vs1 tournament mode ha been ' .. ternary(tournament1vs1_mode, 'enabled', 'disabled') .. ' by ' .. player.name
    )
...
Event.add(defines.events.on_player_changed_position, function(event)
    ...
    storage.tournament1vs1_chart_tags[player.name] = game.forces.spectator.add_chart_tag(player.physical_surface, {
